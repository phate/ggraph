...
test(const std::vector<std::string>&)
{
	...
	size_t mon = max_open_nodes(*root);
	assert(mon == 7);

	return 0;
}

GGRAPH_TEST_REGISTER("unit_tests/aggregation/test-mon", {}, test);
